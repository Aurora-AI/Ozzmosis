...
export const shieldConfigSchema = {
  type: "object",
  properties: {
...
function requireBearer(authHeader: string | null, shieldToken?: string | null) {
...
export function createShieldApp(config: ShieldConfig = {}) {
  const shieldToken = config.token ?? process.env.SHIELD_TOKEN;

  return new Elysia({ adapter: node() })
    .get("/health", () => ({ ok: true }))
    .get("/proxy/tasks", ({ request, set }) => {
      const auth = requireBearer(request.headers.get("authorization"), shieldToken);
      if (!auth.ok) {
        set.status = 403;
        return auth;
      }
...
export { verifyToken } from "./core/verifier";
export type { ShieldDecision } from "./core/verifier";
