...
export function AuditHeader({ sessionId, productContext, timestamp }: AuditHeaderProps) {
  ...
  return (
    <header className="border-b" style={{ borderColor: "var(--color-border)", height: "64px" }}>
      <div className="g-container h-full flex items-center justify-between">
        <div>
          ...
          {productContext && (
            <p className="mt-0.5" style={{ fontSize: "var(--text-xs)", color: "var(--color-fg)", opacity: 0.6 }}>
              {productContext}
            </p>
          )}
        </div>
        ...
        <div className="text-right">
          {sessionId && (
            <p className="font-mono" style={{ fontSize: "var(--text-xs)", color: "var(--color-fg)", opacity: 0.5 }}>
              {sessionId}
            </p>
          )}
          <p className="font-mono mt-0.5" style={{ fontSize: "var(--text-xs)", color: "var(--color-fg)", opacity: 0.4 }}>
            {displayTimestamp}
          </p>
        </div>
      </div>
