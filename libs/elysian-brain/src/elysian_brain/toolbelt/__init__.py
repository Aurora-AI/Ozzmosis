...
from .tools import calculator, finance, stats, calendar, reference, search_local


def build_registry() -> ToolRegistry:
    reg = ToolRegistry()
    reg.register(calculator.spec(), calculator.run)
    reg.register(finance.spec(), finance.run)
    reg.register(stats.spec(), stats.run)
    reg.register(calendar.spec(), calendar.run)
